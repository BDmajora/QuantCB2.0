...
// Internal Imports
use crate::training::trainer::trainer_config::{TrainingConfig, TAG_TRUTH, TAG_HALLUCINATE, TAG_SHAKESPEARE, TAG_WIKI}; 
...
impl<B: AutodiffBackend, O: Optimizer<QuantCB<B>, B>> QuantCBTrainer<B, O> {
    ...
    pub fn train_step(&mut self, batch: QuantCBBatch<B>) -> f32 {
        ...
        self.model = self.optimizer.step(self.lr, self.model.clone(), burn::optim::GradientsParams::from_grads(grads, &self.model));
...
pub async fn run() {
    ...
    let mut config = TrainingConfig::new(QuantCBConfig::new(), burn::optim::AdamConfig::new());
    ...
    // 2. Model/Optim Initialization (Handled by the CheckpointManager)
    let (model, optimizer) = checkpoints.load_or_init::<TrainBackend>(&config, &device);
    
    // LOAD STATE (Fix: Retrieving mtp_weight and wake_ups from checkpoint)
    ...
    // Resume scheduler state (Fix: Restoring MTP memory)
    if step > 0 { 
    ...
    loop {
        ...
        if let Some(batch_items) = streamer.get_next_batch() {
            ...
            // Sync scheduler decisions to trainer
            trainer.lr = state.lr;
            trainer.mtp_loss_weight = state.mtp_weight;
            
            if step % 10 == 0 {
                let tps = (config.batch_size * config.seq_len * 10) as f64 / t0.elapsed().as_secs_f64().max(0.001);
                
                // Keep this one (the pretty version)
                println!(
                ...
                // println!("DEBUG_STATE: {},{},{},{}", step, trainer.lr, trainer.mtp_loss_weight, scheduler.wake_ups);
            ...
            if step % 500 == 0 {
                ...
                // SAVE FIX: Writing mtp_weight and wake_ups to disk
                checkpoints.save(
                    &trainer.model, 
                    &trainer.optimizer, 
                    step, 
...
fn run_sample_block<B: AutodiffBackend>(
    ...
    let output = TextGenerator::generate(model, tokenizer, device, &prompt, 60, 0.8, 1.2);
